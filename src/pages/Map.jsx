...
import { StarIcon, Trash2Icon, XIcon } from "lucide-react";
...
export const Map = () => {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <div className="absolute bottom-0 left-0 w-full z-[10]"><Navbar setIsFavouriteList={() => setIsFavouriteList(!isFavouriteList)}/></div>
      ...
      {isFavouriteList && (
        ...
        <div className="bg-white rounded w-[90%] h-[80%] z-[999] fixed top-[5%] left-[5%] opacity-[1] overflow-y-auto -x-hidden">
          ...
          <div className="flex flex-col gap-1 px-4 sm:px-12 py-8">
            {markers.length > 0 && markers.map(point => (
              <div key={point.id} className="cursor-pointer w-full border rounded border-gray-400 flex justify-between items-center px-4 py-2" onClick={() => {
                ...
                <div onClick={(event) => event.stopPropagation()}>
                  <Trash2Icon onClick={() => handleDeleteListFavouritesPoint(point.id)}/>
                </div>
              </div>
            ))}
          </div>
